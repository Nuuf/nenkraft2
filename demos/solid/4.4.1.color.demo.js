...
export default () => {
  ...
  CreateDemo( 'Color', ( conf ) => {
    ...
    const img = nk2.BrowserUtility.ImageFromDataURL( 
      nk2.BrowserUtility.GenerateSimpleBase64PNG( () => {
        ...
        for ( var i = 0; i < 260; ++i ) {
          ...
          path.style.fill.color = new nk2.Color( i, 0, 0, 1 )
            .Mix( prevColor, 0.6 )
            .ComputeValueRGBA().value;
          path.style.stroke.color = new nk2.Color( 0, 255 - i, 0, 1 )
            .Mix( prevColor, 0.9 )
            .ComputeValueRGBA().value;
        ...
        scene.AddChild( 
          new nk2.Sprite( 
            -HW, -HH, new nk2.Texture.BasicTexture2D( img, null, img.width, img.height )
          )
        );
